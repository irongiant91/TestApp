(function (window) {
  ...
  NOVA.cbs_uom_list = ko.observableArray([]);
  ...
  var categoryItem = function(){
    ...
    this.msp = ko.observable('');
    this.rsp = ko.observable('');
    this.order = ko.observable('');
    this.is_deletable = ko.observable('');
    this.fill = function (d) {
  ...
  NOVA.getCbsUomList = function(){
    var csrftoken = NOVA.getCookie('csrftoken');
    $.ajax({
      method: 'GET',
      url: '/api/admin/cbs/uom/list/get',
    ...
    .done( function (d, textStatus, jqXHR) {
      ...
      for(var i=0; i< d.data.length; i++){
        NOVA.cbs_uom_list.push(d.data[i]);
      }
  ...
  $('.category-create-btn').on('click', function(){
    $('.contract-services-wrapper').removeClass('d-none');
    NOVA.categoryName('');
    NOVA.categoryItems([]);
  ...
  NOVA.saveCategory = function(){
    ...
    $.ajax({
      method: 'POST',
      url: '/api/admin/cbs/category/create',
      data: formdata,
      contentType: false,
      processData: false,
      beforeSend: function(xhr, settings) {
    ...
    .done( function (d, textStatus, jqXHR) {
      $('.contract-services-wrapper').addClass('d-none');
      NOVA.showToast(d);
      NOVA.getCbsCategoriesList();
    })
    .fail( function (jqXHR, textStatus, errorThrown) {
  ...
  NOVA.init = function(){
    ...
    $('.contract-service-form').each(function(){
      getId = $(this).attr('id');
      $("#"+getId).validate({
        ignore: "",
        errorElement: 'span',
    ...
    /*$.validator.addMethod("minimumSellPriceRequired", $.validator.methods.required, "Please enter minimum sell price");
    $.validator.addClassRules("minimum-sell-price", {
      minimumSellPriceRequired: true,
      dollarsscents: true  
    });

    $.validator.addMethod("recommendedPriceRequired", $.validator.methods.required, "Please enter recommended price");
    $.validator.addClassRules("recommended-price", {
      recommendedPriceRequired: true,
      dollarsscents: true  
    });*/
  }
  ...
  NOVA.editCategory = function(){
    ...
    $.ajax({
      method: 'POST',
      url: '/api/admin/cbs/category/edit',
      data: formdata,
      contentType: false,
      processData: false,
      beforeSend: function(xhr, settings) {
  ...
  NOVA.deleteCategory = function(data, e){
    ...
    $.ajax({
      method: 'POST',
      url: '/api/admin/cbs/category/delete',
      data: formdata,
      contentType: false,
      processData: false,
      beforeSend: function(xhr, settings) {
  ...
  NOVA.disableCategory = function(category_id){
    ...
    $.ajax({
      method: 'POST',
      url: '/api/admin/cbs/category/disable',
      data: formdata,
      contentType: false,
      processData: false,
      beforeSend: function(xhr, settings) {
  ...
  NOVA.enableCategory = function(category_id){
    ...
    $.ajax({
      method: 'POST',
      url: '/api/admin/cbs/category/enable',
      data: formdata,
      contentType: false,
      processData: false,
      beforeSend: function(xhr, settings) {
  ...
  $(document).on('show.bs.collapse','.collapse', function () {
    ...
    $(this).parent().parent().find('.item-delete-btn').attr('disabled','disabled');
    $(this).parent().parent().find('.item-add-btn').attr('disabled','disabled');
