(function (window) {
  ...
  $('#uploadContractModal').on('hidden.bs.modal', function () {
    ...
    $('[name="refNumber"]').on('keyup', function() {
      var noSPace = $(this).val().match(/^[a-z,0-9]/i);
      if($(this).val() == '' || noSPace === null) {
        $('#uploadContractFile').css({'opacity': .6}).removeClass('cursor-pointer').find('input').prop('disabled', true);
      } else {        
  ...
  NOVA.viewExpenseConfig = function(){
    window.location = '/client/'+NOVA.client_id()+'/contract-detail/'+NOVA.contract_id()+'/expense_config'
  }
...
function init() {
  if (document.readyState == "interactive") {
    ...
    var contract_id = docUrlArr[docUrlArr.length - 1];
