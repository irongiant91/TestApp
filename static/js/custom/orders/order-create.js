(function (window) {
  ...
  NOVA.contract_end_date = ko.observable('');
  ...
  NOVA.getJobTypesList = function(){
    ...
    $.ajax({
      ...
      data: {'client_id': NOVA.client_id(), 'job_start_date': NOVA.job_start_date()},
      beforeSend: function(xhr, settings) {
        NOVA.showLoading();
        xhr.setRequestHeader("X-CSRFToken", csrftoken);
      }
    ...
    .done( function (d, textStatus, jqXHR) {
      ...
      NOVA.contract_end_date(d.contract_end_date);
    })
    .fail( function (jqXHR, textStatus, errorThrown) {
      NOVA.jobtypes_list([]);
      NOVA.vehicles_list([]);
  ...
  NOVA.jobtypeChangeEvent = function(data, e){
    ...
    NOVA.jobtypes_list().forEach(function(entry) {
      ...
      if(entry.jobtype_id == data.jobtype_id){
        ...
        for(var i=0; i<entry.documents_list.length; i++){
          var document1 = new documentInput();
          document1.fill(entry.documents_list[i]);
        ...
        if(entry.service_type == 'Charter'){
          ...
          $('#recurrence_modal').removeClass('d-none');
  ...
  NOVA.saveRecurrenceClick = function(){
    ...
    $('#recurrance-update-btn').removeClass('d-none');
    $('#recurrance-save-btn').addClass('d-none');
  }

  NOVA.resetRecurrenceClick = function(){
    NOVA.has_recurrence('false');
    NOVA.recurrence_name('');
    $('[name="period"]').prop('checked', false);
    $('#recurrence_month, #recurrence_week, #recurrence_day, [name="endDateModal"]').val('');
    $('#month-list, #week-list, #day-list, #end-date').addClass('d-none');
    $('#recurrenceModal').modal('hide');
    $('#recurrance-update-btn').addClass('d-none');
    $('#recurrance-save-btn').removeClass('d-none');
  }
  ...
  NOVA.saveOrderConfig = function(){
    ...
    /*var finance_selected = [];
    $.each($("input[name='jobFinance']:checked"), function(){
      finance_selected.push($(this).val());
    });*/
    ...
    formdata.append('contract_end_date', NOVA.contract_end_date());
    formdata.append('job_end_time', $('#job_end_time').val());
    $.ajax({
      method: 'POST',
      url: '/api/order/order/config/create',
      data: formdata,
      contentType: false,
      processData: false,
      beforeSend: function (xhr, settings) {
